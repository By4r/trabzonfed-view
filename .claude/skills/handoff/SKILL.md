...
Trabzon Federasyonu projesi (trabzonfed-view) oturum kapanış akışı: bu oturumda çalışılan
dosyaları seçici olarak commit'le, origin/main'e push'la, `tasks/handoff.md`'yi güncelle,
kapanış raporu ver.

**Önemli ilke:** Beyar bu skill'i çağırdığında commit + push izni verilmiş sayılır; ayrıca izin
sorulmaz. (CLAUDE.md'deki "commit'i handoff'a bırak" kuralının açık izni budur.)

**Remote notu:** Hedef repo `By4r/trabzonfed-view` (public, GitHub Pages). Remote henüz
kurulmadıysa ilk handoff'ta kur: `gh repo create By4r/trabzonfed-view --public --source=. --push`
veya mevcut repo varsa `git remote add origin git@github.com:By4r/trabzonfed-view.git`.
Remote kurulumu da bloke edilirse BLOKE FALLBACK'e düşür.
...
3. **Exclude listesi (bu projeye özgü; bu path'ler ASLA commit'e girmez):**
   - `tasks/` klasörünün tamamı — `handoff.md`, `plan.md`, `components.md` ve sonradan eklenen
     her tasks dosyası (gitignore kuralı: `tasks/`)
   - `docs/screenshots/` altındaki her şey — QA screenshot'ları (gitignore kuralı: `docs/screenshots/`)
   - `.claude/` altı — SADECE `.claude/skills/` takiptedir, gerisi girmez
